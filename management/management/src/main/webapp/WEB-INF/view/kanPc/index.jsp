...
function executeQuote(Obj){
	...
	insert.admin.value = Obj.form.adminNumber.value;
	insert.user.value = Obj.form.userNumber.value;
...
<s:form styleId="insert">
	<table border="1">
		...
		<tr>
			...
			<c:if test="${empty assetsMode}">
					<option value="" selected>選択して下さい</option>
			</c:if>
			<c:if test="${!empty assetsMode}">
					<option value="${f:h(assetsMode)}">${assetsMode}</option>
			</c:if>
		...
		<tr>
			<td><select name="maker">
				...
				<c:if test="${!empty maker}">
					<option value="${f:h(maker)}">${maker}</option>
				</c:if>
			...
			<c:if test="${empty wirelessLan}">
					<option value="" selected>選択して下さい</option>
			</c:if>
			<c:if test="${!empty wirelessLan}">
					<option value="${f:h(wirelessLan)}">${wirelessLan}</option>
			</c:if>
...
<s:form>
	...
	<c:forEach var="a" items="${pcMasterDto}" varStatus="st">
		...
		<tr bgcolor="aliceblue">
			...
			<html:hidden property="adminNumber" value="${f:h(a.adminNumber)}" />
			<html:hidden property="user" value="${f:h(a.user)}" />
			<html:hidden property="userNumber" value="${f:h(a.userNumber)}" />
