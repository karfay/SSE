...
public class ThemeMaster implements Serializable {
    ...
    /**
     * themeMasterとthemeGroupを紐付けたいために宣言している。
     * s2jgbcのgen-ddlを使うとテーママスタのテーマ親番に外部キー制約（参照元：原価管理テーブル）が設定されてしまうため、migrateでエラーとなる。
     * そのため、以下のファイルから外部制約にあたるsqlを削除するなりコメントアウトするなりしてからmigrateする必要がある。
     * db/migrate/バージョン番号/create/050-foreignkey/theme_master.sql
     * 環境構築とかの話なのでよくわからない人は気にしないでください。
     */
    @ManyToOne
