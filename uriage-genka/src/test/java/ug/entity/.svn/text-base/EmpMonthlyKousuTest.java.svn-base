...
/**
 * {@link EmpMonthlyKousu}のテストクラスです。
 * 
 */
...
public class EmpMonthlyKousuTest extends S2TestCase {
    ...
    public void testFindById() throws Exception {
        jdbcManager.from(EmpMonthlyKousu.class).id(1).getSingleResult();
    }
    ...
    public void testLeftOuterJoin_themeMaster() throws Exception {
        jdbcManager.from(EmpMonthlyKousu.class).leftOuterJoin(themeMaster()).id(1).getSingleResult();
    }
    ...
    public void testLeftOuterJoin_empMaster() throws Exception {
        jdbcManager.from(EmpMonthlyKousu.class).leftOuterJoin(empMaster()).id(1).getSingleResult();
    }
}